	...
	for (i=start; i<=stop; i++) {
				...
				//option 1 + consider all subotimal structures:
				 ...
				 //to have no structure
				  ...
				  for (k=1;k<=ct->numofstructures;k++) {
		          		...
		          		fn = exp(-(((long double)energyarray[k]-energyarray[1])/(rt*conversionfactor)));
					   sn = sn + fn;
		...
		//option 2: refold  rna
			...
			if (foldsize ==0) {
				...
				//option 2 +notrefold+ consider all suboptimal structures with ensemble energy
					...
					Qc = 0;
					for (k=1;k<=ct->numofstructures;k++) {
	          			fn = exp(-(((long double)ctenergy[k]-energyarray[1])/(rt*conversionfactor)));
						Qc = Qc + fn;
         			}
					//final free energy difference					
					table[i][2] = (int)( conversionfactor*rt*log( (long double)Q/(long double)Qc ) );
				...
				//option 2 +notrefold+ consider heuristic suboptimal structures with average energy
					...
					sn = 0;
					for (k=1;k<=ct->numofstructures;k++) {
	          			fn = exp(-(((long double)ct->energy[k]-ct->energy[1])/(rt*conversionfactor)));
						sn = sn + fn;
				...
				//option 2 +notrefold+ partionfunction
					...
					table[i][2] = (int)(conversionfactor*rt* 
						( (long double)(ct->numofbases)*log(rescaleinrefill) 
			...
			//scan: only folding the region close to the binding site
				...
				//option 2 + refold + consider heuristic suboptimal structures average free energy
					...
					sn = 0;
					for (k=1;k<=fracct->numofstructures;k++) {
	          				fn = exp(-(((long double)fracct->energy[k]-fracct->energy[1])/(rt*conversionfactor)));
							sn = sn + fn;
				...
				//option 2 + refold + partionfunction
					...
					table[i][2] = (int)(conversionfactor*rt*
					( (long double)(fracct->numofbases)*log(rescaleinrefill) 
						+ log((long double)Q/(long double)Qc) ) );
				}
				//---------------------------------------------------------------------------------------------
		...
		if (Usesub==2) {	//-2 is unavailabe, as partition function is not availabe for inter oligo	
			...
			oligo->energy[1]= (int)( conversionfactor*rt*
				( (long double)(oligo->numofbases)*log((interoligo->data)->scaling)  - log((long double)Qolig) ) );
		}
		else {
		...
		if ((table[i][2]<0)&&(dgeff<0)) {
	   		table[i][0] = table[i][1] + (int)((rt*conversionfactor)*log( (exp(-(long double)(table[i][2])/
						  (rt*conversionfactor))+1.0)*  (exp(-(long double)(dgeff)/(rt*conversionfactor))+1.0) )+0.5);
		}
		else if (table[i][2]<0) {
			table[i][0] = table[i][1] + (int)((rt*conversionfactor)*log( (exp(-(long double)(table[i][2])/
			...
			table[i][0] = table[i][1] + (int)((rt*conversionfactor)*log(  (exp(-(long double)(dgeff)/
						  (rt*conversionfactor))+1.0) )+0.5);
		}
...
out << "Oligo walk for:  "<<ct->ctlabel[1]<<"\n\n";
if (mask!=NULL) {
	out << "Oligonucleotides filtered by siRNA selection criteria: ";
...
if (Usesub) out << "Suboptimal structures were used.  There are "<<ct->numofstructures<<" suboptimal structures.";
