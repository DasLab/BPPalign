...
double Dynalign_object::GetBestPairEnergy(const int sequence,const int a, const int b) {
	...
	if (array==NULL) {
		...
		array = new double **[2];
		array[0] = new double *[GetRNA1()->GetStructure()->numofbases+1];
		for (i=0;i<=GetRNA1()->GetStructure()->numofbases;i++) {
		...
		array[1] = new double *[GetRNA2()->GetStructure()->numofbases+1];
		for (i=0;i<=GetRNA2()->GetStructure()->numofbases;i++) {
		...
		for (i=1;i<=GetRNA1()->GetStructure()->numofbases;i++) {
			for (j=i+minloop;j<=GetRNA1()->GetStructure()->numofbases;j++) {
				for (k=max(lowend[i],1);k<=(min(GetRNA2()->GetStructure()->numofbases,highend[i]));k++) {
					for (l=max(lowend[j],k);l<=(min(GetRNA2()->GetStructure()->numofbases,highend[j]));l++) {
						...
						array[0][j][i]=min(array[0][j][i],((double) (v->f(i,j,k,l)+v->f(j,i+GetRNA1()->GetStructure()->numofbases,l,k+GetRNA2()->GetStructure()->numofbases)))/conversionfactor);
						
						//allow single BP inserts
						if (i>1&&j<GetRNA1()->GetStructure()->numofbases&&k>1&&l<GetRNA1()->GetStructure()->numofbases&&k+1>=lowend[i+1]&&k+1<=highend[i+1]&&k<=highend[i-1]&&k>=lowend[i-1]
							&&l>=lowend[j+1]&&l<=highend[j+1]&&l-1<=highend[j-1]&&l-1>=lowend[j-1]) {
							array[0][j][i]=min(array[0][j][i],((double) (v->f(i+1,j-1,k+1,l-1)+v->f(j+1,(i-1)+GetRNA1()->GetStructure()->numofbases,l,k+GetRNA2()->GetStructure()->numofbases)+
						...
						array[1][l][k]=min(array[1][l][k],((double) (v->f(i,j,k,l)+v->f(j,i+GetRNA1()->GetStructure()->numofbases,l,k+GetRNA2()->GetStructure()->numofbases)))/conversionfactor);

						//allow single BP inserts
						if (i>1&&j<GetRNA1()->GetStructure()->numofbases&&k>1&&l<GetRNA2()->GetStructure()->numofbases&&k+1>=lowend[i+1]&&k+1<=highend[i+1]&&k-1<=highend[i]&&k-1>=lowend[i]
							&&l+1>=lowend[j]&&l+1<=highend[j]&&l-1<=highend[j-1]&&l-1>=lowend[j-1]) {
							array[1][l][k]=min(array[1][l][k],(double) ((v->f(i+1,j-1,k+1,l-1)+v->f(j,i+GetRNA1()->GetStructure()->numofbases,l+1,k-1+GetRNA2()->GetStructure()->numofbases)+
...
void Dynalign_object::AllocateForceAlign() {
	...
	forcealign[0]=new short [GetRNA1()->GetStructure()->numofbases+1];
	forcealign[1]=new short [GetRNA2()->GetStructure()->numofbases+1];
	for (index=1;index<=GetRNA1()->GetStructure()->numofbases;index++) {
