...
class RNA: public Thermodynamics {
	...
	public:
		...
		double CalculateFreeEnergy(const int structurenumber = 1);
		...
		int WriteThermodynamicDetails(const char filename[]);
		...
		int FoldSingleStrand(const float percent, const int maximumstructures, const int window, const char savefile[]="", const int maxinternalloopsize = 30);
		...
		int ReadSHAPE(const char filename[], const double parameter1, const double parameter2, const bool IsPseudoEnergy=true);
		...
		//!\return An integer that indicates an error code (0 = no error, 1 = input file not found).
		int ReadSHAPE(const char filename[], const double parameter1, const double parameter2, const double ssm, const double ssb);
		...
		int WriteCt(const char filename[]);
		...
		int BreakPseudoknot(bool minimum_energy=true);
