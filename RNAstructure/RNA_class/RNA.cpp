...
double RNA::CalculateFreeEnergy(const int structurenumber) {
...
int RNA::FoldSingleStrand(const float percent, const int maximumstructures, const int window, const char savefile[], const int maxinternalloopsize) {
	char *savefilename;
	int percenti;
	int tracebackstatus;
	...
	tracebackstatus = dynamic(ct, data, maximumstructures, percenti, window, progress, false, savefilename, maxinternalloopsize);
...
int RNA::PartitionFunction(const char savefile[], double temperature) {
	...
	if (ct->shaped) {
		for (i=1;i<=2*ct->numofbases;i++) ct->SHAPE[i]=boltzman((short) ct->SHAPE[i], pfdata->temp);
...
int RNA::ReadSHAPE(const char filename[], const double parameter1, const double parameter2, const bool IsPseudoEnergy) {
	FILE *check;
	...
	if (IsPseudoEnergy) {
		...
		ct->ReadSHAPE(filename);//call ReadSHAPE() to read the file and determine pseudo energies
...
//Returns an integer that indicates an error code (0 = no error, 1 = input file not found).
int RNA::ReadSHAPE(const char filename[], const double parameter1, const double parameter2, const double ssm, const double ssb) {
	FILE *check;
...
int RNA::BreakPseudoknot(bool minimum_energy) {
	...
	if (minimum_energy) {
		...
		for (structures=1;structures<=ct->numofstructures;structures++) {
