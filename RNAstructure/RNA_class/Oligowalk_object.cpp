...
int Oligowalk_object::Oligowalk(const int oligo_length, const bool isDNA, const int option, const double oligo_concentration, const int usesub, const int start, const int stop   ) {
	...
	prefilter = new siPREFILTER(*GetDatatable(),*enthalpy,0,true,GetStructure()->numofbases - oligo_length + 2,isDNA);


	//Allocate the tables needed for storing the results
	table = new int*[GetStructure()->numofbases - oligo_length + 2];

	for (i = 0; i < GetStructure()->numofbases - oligo_length + 2; i++) {
	...
	numofsubstructures= new int*[GetStructure()->numofbases - oligo_length +2];
	for (i = 0; i < GetStructure()->numofbases - oligo_length + 2; i++)	{
	...
	if (isDNA) {
		...
		//set the temperature for the DNA parameters (which also triggers a read of the parameters)
		error = ddata->SetTemperature(GetTemperature());

		//Check fo an error when reading the parameters
		if (error!=0) {
			delete ddata;
			delete prefilter;
			delete helixstack;
...
double Oligowalk_object::GetBreakTargetDG(const int index) {
	...
	else if (index<1||index>GetStructure()->numofbases - length + 1) {
...
double Oligowalk_object::GetTm(const int index) {
	...
	return (((double) table[index][2])/10);
...
Oligowalk_object::~Oligowalk_object() {
	...
	if (table!=NULL) {
		...
		for (i = 0; i < GetStructure()->numofbases - length + 2; i++) {
			delete[] table[i];
