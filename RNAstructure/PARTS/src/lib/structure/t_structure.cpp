...
void t_structure::check_set_label()
{
	...
	for(int i_valid = 0; i_valid < strlen(this->ctlabel); i_valid++)
	{
		for(int i_check = 0; i_check < strlen(invalid_label_chars); i_check++)
		{
...
t_structure::t_structure(char* ct_label, vector<char>* nuc_vector)
{
    this->numofbases = 0;
	this->numseq = NULL;
	this->nucs = NULL;
	...
	this->numofbases = nuc_vector->size();
	...
	for(int i_vect = 0; i_vect < nuc_vector->size(); i_vect++)
	{	
		// Process this nuc.
		if(nuc_vector->at(i_vect) != '\n' && nuc_vector->at(i_vect) != ' ' && nuc_vector->at(i_vect) != '1')
		{
			this->nucs[i_nuc] = nuc_vector->at(i_vect);

			if(nuc_vector->at(i_vect) == 'a' ||
				nuc_vector->at(i_vect) == 'c' ||
				nuc_vector->at(i_vect) == 'g' ||
				nuc_vector->at(i_vect) == 'u' ||
				nuc_vector->at(i_vect) == 't')
			{
				this->unpaired_forced[i_nuc] = true;
			}
			else
			{
				this->unpaired_forced[i_nuc] = false;
			}

			// Convert current base character into number value, from Dave's structure code.
			if (toupper(this->nucs[i_nuc]) == 'A') 
				this->numseq[i_nuc]=1;
			else if (toupper(this->nucs[i_nuc]) == 'C') 
				this->numseq[i_nuc]=2;
			else if (toupper(this->nucs[i_nuc]) == 'G') 
				this->numseq[i_nuc]=3;
			else if (toupper(this->nucs[i_nuc]) == 'U' || toupper(this->nucs[i_nuc]) == 'T') 
				this->numseq[i_nuc]=4;
			else if (toupper(this->nucs[i_nuc]) == 'I') 
				this->numseq[i_nuc]=5;
			else 
				this->numseq[i_nuc]=0; // Map unknown nucleotides to A automatically!
	...
	while(1)
	{
		...
		if(strlen(cur_line) > 0)
		{
			...
			else
			{
				...
				for(int i_cpy = 0; i_cpy < strlen(cur_line); i_cpy++)
				{
...
t_structure::t_structure(char* _fp)
{
...
void t_structure::openct(char* ct_fp)
{
	...
	for(int i = 1; i <= this->numofbases; i++)
	{
		...
		//                1  G 0  2  120 1
		fscanf(ct_file, "%d %c %d %d %d %d", &index, &this->nucs[i], &dangles[i], &stacks[i], &this->basepr[i], &some_val1);
...
bool t_structure::verify_ct(char* ct_fp)
{
	return(true);
}
