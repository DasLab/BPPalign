...
t_ML_result* t_phmm_aln::compute_ML_alignment(int* aligned_positions)
{
	...
	t_phmm_array* ml_array = this->compute_ML_array(ml_res, aligned_positions);
...
*/
t_phmm_array* t_phmm_aln::compute_ML_array(t_ML_result* ml_res, int* seq1_aln_const)
{
		...
		for(int k = min_k; k <= max_k; k++)
		{
			...
			for(int cur_state = 0; cur_state < N_STATES; cur_state++)
			{
				...
				for(int prev_state = 0; prev_state < N_STATES; prev_state++)
				{
					...
					{
						trans_emit_prob = get_trans_emit_prob(prev_state, cur_state, i, k);
	...
	while(i != 0 || k != 0)
	{
		...
		if(i > 0 && k > 0 && cur_state == STATE_ALN)
		{
			...
			for(int i_prev_state = 0; i_prev_state < N_STATES; i_prev_state++)
			{
				...
				if(xlog_comp( ml_array->x(i, k, cur_state), xlog_mul(ml_array->x(i-1, k-1, i_prev_state), this->get_trans_emit_prob(i_prev_state, cur_state, i, k)) ))
				{
					//printf("Found! %d\n", i_prev_state);
		...
		if(i > 0 && cur_state == STATE_INS1)
		{
			...
			for(int i_prev_state = 0; i_prev_state < N_STATES; i_prev_state++)
			{
				...
				if(xlog_comp( ml_array->x(i, k, cur_state), xlog_mul(ml_array->x(i-1, k, i_prev_state), this->get_trans_emit_prob(i_prev_state, cur_state, i, k)) ))
				{
					prev_state = i_prev_state;
				}
			} // prev_state loop
		...
		if(k > 0 && cur_state == STATE_INS2)
		{
			...
			for(int i_prev_state = 0; i_prev_state < N_STATES; i_prev_state++)
			{
				...
				if(xlog_comp( ml_array->x(i, k, cur_state), xlog_mul(ml_array->x(i, k-1, i_prev_state), this->get_trans_emit_prob(i_prev_state, cur_state, i, k)) ))
				{
					prev_state = i_prev_state;
					//printf("Found! %d\n", i_prev_state);
				}
...
if(_DUMP_PHMM_ML_LOOPS_MESSAGES_)
	printf("Similarity = %lf\n", ml_aln->get_aln_similarity('.'));

	ml_res->ml_similarity = ml_aln->get_aln_similarity('.');
