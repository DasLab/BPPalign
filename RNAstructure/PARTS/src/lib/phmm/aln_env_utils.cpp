...
t_aln_env_result* t_phmm_aln::compute_alignment_envelope(int aln_env_type, int par, int* seq1_aln_const)
{
	t_pp_result* pp_result = this->compute_posterior_probs(seq1_aln_const);
	double log_threshold = pp_result->fam_threshold;
	return(compute_alignment_envelope(aln_env_type, pp_result, log_threshold, par, seq1_aln_const));
}
...
if(_DUMP_ALN_ENV_UTILS_MESSAGES_)
	...
	if(_pp_result == NULL)
	{
		pp_result = this->compute_posterior_probs(seq1_aln_const);
	}
	else
	{
