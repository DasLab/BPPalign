...
if(_DUMP_PHMM_PP_LOOPS_MESSAGES_)
	...
	// Compute the arrays.
	this->compute_forward_array(fore_array, seq1_aln_const);
	this->compute_backward_array(back_array, seq1_aln_const);
...
void t_phmm_aln::compute_forward_array(t_phmm_array* fore_array, int* seq1_aln_const)
{
	...
	for(int i = 0; i <= n1; i++)
	{
		...
		{		
			...
			t_phmm_aln::get_aln_permissions(seq1_aln_const, 
											seq2_aln_const,
											forbid_STATE_ALN, 
											forbid_STATE_INS1, 
											forbid_STATE_INS2, 
			...
			for(int current_state = 0; current_state < N_STATES; current_state++)
			{
				...
				for(int prev_state = 0; prev_state < N_STATES; prev_state++)
				{
						...
						fore_array->check_phmm_boundary(i-1, k-1))
					{
						...
						double trans_emit_prob = get_trans_emit_prob(prev_state, current_state, i, k);
						...
						fore_array->check_phmm_boundary(i-1, k))
					{
						...
						//printf("%s -> %s\n", state_names[prev_state), state_names[current_state]);
						double trans_emit_prob = get_trans_emit_prob(prev_state, current_state, i, k);
						//printf("Recursing on (%d, %d) with prob. %f.\n", i-1, k, fore_array->x(i-1,k,prev_state]);
						...
						fore_array->check_phmm_boundary(i, k-1))
					{
						...
						//printf("%s -> %s\n", state_names[prev_state), state_names[current_state]);
						double trans_emit_prob = get_trans_emit_prob(prev_state, current_state, i, k);
						//printf("Recursing on (%d, %d) with prob. %f.\n", i, k-1, fore_array->x(i,k-1,prev_state]);
				...
				//printf("%d, %d, current state: %s, prob = %f (%f)\n", i, k, state_names[current_state), fore_array->x(i,k,current_state), xexp(fore_array->x(i,k,current_state]) );
			} // State loop.
		} // k loop.
	} // i loop.

	if(seq2_aln_const != NULL)
	{
		free(seq2_aln_const);
	}
}
...
void t_phmm_aln::compute_backward_array(t_phmm_array* back_array, int* seq1_aln_const)
{
	// Replicate the constraints for fast processing of constraints.
	int* seq2_aln_const = this->get_seq2_aln_const(seq1_aln_const);
			...
			for(int current_state = 0; current_state < N_STATES; current_state++)
			{
				...
				for(int next_state = 0; !forbid_current_state_comp && next_state < N_STATES; next_state++)
				{
						...
						back_array->check_phmm_boundary(i+1, k+1))
					{
						...
						double trans_emit_prob = get_trans_emit_prob(current_state, next_state, i+1, k+1);
/*
						...
						back_array->check_phmm_boundary(i+1, k))
					{
						...
						//printf("%s -> %s\n", state_names[current_state,  state_names[next_state]);
						double trans_emit_prob = get_trans_emit_prob(current_state, next_state, i+1, k);
						...
						back_array->check_phmm_boundary(i, k+1))
					{
						...
						//printf("%s -> %s\n", state_names[current_state,  state_names[next_state]);
						double trans_emit_prob = get_trans_emit_prob(current_state, next_state, i, k+1);
