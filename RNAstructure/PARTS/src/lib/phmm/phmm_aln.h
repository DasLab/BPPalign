...
public:
	...
	// ML and pp functions.
	void compute_forward_array(t_phmm_array* fore_array, int* aligned_positions);
	void compute_backward_array(t_phmm_array* back_array, int* aligned_positions);
	t_phmm_array* compute_ML_array(t_ML_result* ml_res, int* aligned_positions);
	...
	void load_map_limits_from_map(char* aln_map_fn, int* low_limits, int* high_limits);
	...
	t_pp_result* compute_posterior_probs(int* aligned_positions);
	void* free_pp_result(t_pp_result* pp_result);

	t_ML_result* compute_ML_alignment(int* aligned_positions);
	t_aln_env_result* compute_alignment_envelope(int aln_env_type, int par, int* seq1_aln_const);
	...
	static void get_aln_permissions(int* seq1_aln_const, 
									int* seq2_aln_const, 
