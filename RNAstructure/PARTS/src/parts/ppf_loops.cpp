...
char* t_ppf_loops::GetErrorMsg(const int error_code)
{
...
void t_ppf_loops::init_loops()
{
	...
	{	
		...
		t_pp_result* pp_result = phmm_aln->compute_posterior_probs(NULL);
		...
		else // Utilize the threshold from phmm data.
		{
			...
			aln_env_result = phmm_aln->compute_alignment_envelope(PROB_ALN_ENV, 0, NULL);
