...
#include "../lib/phmm/phmm.h"
#include "../lib/phmm/phmm_aln.h"
#include "../lib/phmm/phmm_array.h"
#include "ppf_math.h"
...
#include "../lib/structure/folding_constraints.h"
...
{	
	...
	t_aln_env_result* aln_env_result = phmm_aln->compute_alignment_envelope(PROB_ALN_ENV, 0, NULL);
	t_pp_result* pp_result = aln_env_result->pp_result;
	...
	double current_log_threshold = aln_env_result->pp_result->fam_threshold;
	...
	while(((per_array_mem * N_ARRAYS) + spf1->n_bytes_alloced + spf2->n_bytes_alloced + aln_priors->n_bytes_alloced) > array_mem_limit)
	{
		...
		// probability plane.
		aln_env_result = phmm_aln->compute_alignment_envelope(PROB_ALN_ENV, pp_result, current_log_threshold, 0, NULL);		
