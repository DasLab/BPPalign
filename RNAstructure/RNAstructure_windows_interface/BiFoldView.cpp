...
void CBiFoldView::OnStart() 
{
	...
	if((openseq (&(GetBiFoldDocument()->ct),m_sequence1.GetBuffer(10)))==0||
		(openseq (&ct2,m_sequence2.GetBuffer(10)))==0) {
   ...
   for (i=1;i<=GetBiFoldDocument()->ct.nnopair;i++) {
		ct3.nnopair++;
		ct3.nopair[ct3.nnopair] = GetBiFoldDocument()->ct.nopair[i];
   ...
   if (forbidunimolecular) {
	   ...
	   ct3.allocatetem();
	   for (i=1;i<GetBiFoldDocument()->ct.numofbases;i++) {
		   for (j=i+1;j<=GetBiFoldDocument()->ct.numofbases;j++) {
				ct3.tem[j][i]=false;
		   }

	   }
	   for (i=GetBiFoldDocument()->ct.numofbases+3;i<ct3.numofbases;i++) {
		   for (j=i+1;j<=ct3.numofbases;j++) {
				ct3.tem[j][i]=false;
		   }
