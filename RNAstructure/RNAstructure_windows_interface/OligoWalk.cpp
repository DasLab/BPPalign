...
UINT OligoProc( LPVOID pParam ){    
	...
	prefilter = new siPREFILTER(oligoobject->data,oligoobject->dhdata,oligoobject->useprefilter,true,oligoobject->ct.numofbases - oligoobject->length + 2,oligoobject->isdna);
...
void COligoWalk::OnCtbutton() 
{
	...
	if (filedialog->DoModal()==IDOK) {
		...
		openct(&(GetOligoDocument()->oligoobject->ct),m_ctname.GetBuffer(0));
		...
		m_start = 1;
		m_stop = GetOligoDocument()->oligoobject->ct.numofbases;
		ctloaded = true;

		if (GetOligoDocument()->oligoobject->ct.numofbases<m_length) 
			m_length = GetOligoDocument()->oligoobject->ct.numofbases;
...
void COligoWalk::OnLengthSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(ctloaded) {
		...
		if (m_length>=GetOligoDocument()->oligoobject->ct.numofbases) { 
			m_length=GetOligoDocument()->oligoobject->ct.numofbases;
...
void COligoWalk::OnStartSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (ctloaded) {
		...
		if (m_start>=GetOligoDocument()->oligoobject->ct.numofbases) { 
			m_start=GetOligoDocument()->oligoobject->ct.numofbases;
...
void COligoWalk::OnStopSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (ctloaded) {
		...
		if (m_stop>=GetOligoDocument()->oligoobject->ct.numofbases) { 
			m_stop=GetOligoDocument()->oligoobject->ct.numofbases;
