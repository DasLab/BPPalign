...
void CColorKey::OnDraw(CDC* pDC)
{
	...
	pDC->TextOut(0, y, "BP Probability >= 99%");
	y+=3*KEYFONTHEIGHT/2;

	SetTextColor(pDC->m_hDC ,crColor[2]);
	pDC->TextOut(0, y, "99% > BP Probability >= 95%");
	y+=3*KEYFONTHEIGHT/2;

	SetTextColor(pDC->m_hDC ,crColor[3]);
	pDC->TextOut(0, y, "95% > BP Probability >= 90%");
	y+=3*KEYFONTHEIGHT/2;
	...
	pDC->TextOut(0, y, "80% > BP Probability >= 70%");
	y+=3*KEYFONTHEIGHT/2;

	SetTextColor(pDC->m_hDC ,crColor[6]);
	pDC->TextOut(0, y, "70% > BP Probability >= 60%");
	y+=3*KEYFONTHEIGHT/2;

	SetTextColor(pDC->m_hDC ,crColor[7]);
	pDC->TextOut(0, y, "60% > BP Probability > 50%");
	y+=3*KEYFONTHEIGHT/2;
