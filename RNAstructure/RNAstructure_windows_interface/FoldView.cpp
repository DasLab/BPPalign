...
void currentconstraints(structure *ct) {
   ...
   if (ct->ndbl>0) {
      ...
      for (it=1;it<=ct->ndbl;it++) {
      	itoa(ct->dbl[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
   ...
   if (ct->npair>0) {
   	...
   	for (it = 1; it<=ct->npair;it++) {

 		itoa(ct->pair[it][0],temp,10);
         strcat(message,"  ");
         strcat(message,temp);
         strcat(message," - ");
         itoa(ct->pair[it][1],temp,10);
         strcat(message,temp);
         if (it%5==0) strcat(message,"\n");
		 else strcat(message,", ");
      }
   ...
   if (ct->nnopair>0) {
      ...
      for (it=1;it<=ct->nnopair;it++) {
      	itoa(ct->nopair[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
         if (it%5==0) strcat(message,"\n");
		 else strcat(message,", ");
   ...
   if (ct->ngu>0) {
      ...
      for (it=0;it<ct->ngu;it++) {
      	itoa(ct->gu[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
   ...
   if (ct->nmod>0) {
      ...
      for (it=1;it<=ct->nmod;it++) {
      	itoa(ct->mod[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
         if (it%5==0) strcat(message,"\n");
		 else strcat(message,", ");
   ...
   if (ct->nforbid>0) {
		...
		for (it = 0; it<ct->nforbid;it++) {

 			itoa(ct->forbid[it][0],temp,10);
			strcat(message,"  ");
			strcat(message,temp);
			strcat(message," - ");
			itoa(ct->forbid[it][1],temp,10);
			strcat(message,temp);
			if (it%5==0) strcat(message,"\n");
			else strcat(message,", ");
		}
   ...
   for (i=0;i<ct->nmicroarray;i++) {
	...
	if (ct->ndbl==0&&ct->npair==0&&
		ct->nnopair==0&&ct->ngu==0&&
		ct->nmod==0&&ct->min_gu==0&&
		ct->min_g_or_u==0&&ct->nneighbors==0
		&&ct->nforbid==0&&ct->nregion==0&&ct->nmicroarray==0) {
...
void CFoldView::NewSequence() 
{
	...
	//load the ct file:
	if((openseq (&(GetFoldDocument()->ct),m_sequencename.GetBuffer(10)))==0) {
   	//errmsg(5,5);
   }
   if ((GetFoldDocument()->ct.numofbases)>1200) {
   			m_window=20;
   }
   else if ((GetFoldDocument()->ct.numofbases)>800) {
   			m_window=15;
            
   }
   else if ((GetFoldDocument()->ct.numofbases)>500) {
   			m_window=11;
            
   }
   else if ((GetFoldDocument()->ct.numofbases)>300) {
   			m_window=7;
            
   }
   else if ((GetFoldDocument()->ct.numofbases)>120) {
   			m_window=5;
            
   }
   else if ((GetFoldDocument()->ct.numofbases)>50) {
   			m_window=3;
