...
void CPlot::OnInitialUpdate()
{
	...
	if (rect.bottom<rect.right) {
		...
		zoom = int (100.0*(((float) (rect.bottom)) / ((float) (plotdoc->ct.numofbases*dotsize+topmargin+bottommargin))) );
		
	}
	else zoom = int (100.0*(((float) (rect.right)) / ((float) (plotdoc->ct.numofbases*dotsize+topmargin+bottommargin))) );
	
	if (zoom>250) zoom =250;

	PageSize = new CSize(int ((float)(plotdoc->ct.numofbases*dotsize+topmargin+bottommargin)*(((float) (zoom)/(float) (100)))),
   		int ((float)(plotdoc->ct.numofbases*dotsize+topmargin+bottommargin)*(((float) (zoom)/(float) (100)))));
	WindowSize= new CSize(plotdoc->ct.numofbases*dotsize+topmargin+bottommargin,plotdoc->ct.numofbases*dotsize+topmargin+bottommargin);
	...
	plotsize = (plotdoc->ct.numofbases+1)*dotsize;
...
void CPlot::Draw(CDC* pDC) {
	...
	for (i=10;i<plotdoc->ct.numofbases;i+=10) {
	...
	for (i=10;i<=plotdoc->ct.numofbases-5;i+=10) {
	...
	itoa(plotdoc->ct.numofbases,number,10);
	string = number;
	pDC->TextOut(leftmargin+(plotdoc->ct.numofbases-1)*dotsize-fontleftshift, topmargin-FontSize-scalespace, string);
	pDC->TextOut(leftmargin+plotsize+extlinewidth+scalespace,topmargin+extlinewidth+(plotdoc->ct.numofbases-1)*dotsize-FontSize/2,string);
	...
	for (i=plotdoc->xstart;i<=plotdoc->xstop;i++) {
		for (j=max(i+1,plotdoc->ystart);j<=plotdoc->ystop;j++) {
			for (k=1;k<=plotdoc->colors;k++) {
				if (plotdoc->array[j][i]<=plotdoc->colorrange[k]&&plotdoc->array[j][i]>=plotdoc->colorrange[k-1]) {
...
afx_msg void CPlot::OnLButtonDown( UINT nFlags, CPoint point ) {
	...
	if (i>0&&i<=plotdoc->ct.numofbases&&j>0&&j<=plotdoc->ct.numofbases) {
		string = "BP ";
		itoa(i,number,10);
		string+=number;
		string+="(";
		...
		string += plotdoc->message;
		string+="= ";
		if (plotdoc->array[j][i]>INFINITE_ENERGY/10) {
			strcpy(number,plotdoc->outside);	
		}
		else gcvt(plotdoc->array[j][i],6,number);
		string+=number;
...
void CPlot::OnOutputplot()
{
	...
	if (filedialog->DoModal()==IDOK) {
		...
		out << plotdoc->ct.numofbases<<"\n";
		out << "i\tj\tkcal/mol\n";
		for (int i=plotdoc->xstart;i<=plotdoc->xstop;i++) {
			for (int j=max(i+1,plotdoc->ystart);j<=plotdoc->ystop;j++) {
				
				if (plotdoc->array[j][i]<=plotdoc->colorrange[plotdoc->colors]&&plotdoc->array[j][i]>=plotdoc->colorrange[0]) {
					out << i << "\t" << j << "\t" << plotdoc->array[j][i] << "\n"; 
