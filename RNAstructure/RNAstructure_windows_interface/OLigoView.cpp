...
int COligoView::DGOut(CDC* pDC, int x, int y, bool draw) {
	...
	if (draw) {
		...
		TextOut("37",pDC,x+totalwidth,y+fontsize/2,true);
		pDC->SelectObject(oldFont);
...
void COligoView::OnDraw(CDC* pDC)
{
	...
	while (width+TextOut("G",pDC,width,sequenceline,false)<rect.right&&
		i<=GetDocument()->oligoobject->ct.numofbases) {

		if (GetDocument()->oligoobject->ct.basepr[1][i]>0) pDC->SetTextColor(PAIRED_COLOR);
		switch(GetDocument()->oligoobject->ct.numseq[i]) {
			case (0) :
		...
		if (GetDocument()->oligoobject->ct.basepr[1][i]>0) pDC->SetTextColor(0);
		i++;
	...
	while (width+columnwidth<rect.right&&
		i<=GetDocument()->oligoobject->ct.numofbases-GetDocument()->oligoobject->length+1) {
...
void COligoView::OnRight() {
	...
	current++;
	if (current>GetDocument()->oligoobject->ct.numofbases-GetDocument()->oligoobject->length+1) 
		current=GetDocument()->oligoobject->ct.numofbases-GetDocument()->oligoobject->length+1;
	InvalidateRect(NULL);
	UpdateWindow();
...
void COligoView::OnGraphFreeenergyOverall() 
{
	...
	for (i=1;i<=GetDocument()->oligoobject->ct.numofbases-GetDocument()->oligoobject->length+1;i++) {
...
afx_msg void COligoView::OnLButtonDown( UINT nFlags, CPoint point ) {
	...
	if (point.y>sequenceline) {
		...
		if (current<1) current = 1;
		else if (current>GetDocument()->oligoobject->ct.numofbases-GetDocument()->oligoobject->length+1)
			current = GetDocument()->oligoobject->ct.numofbases-GetDocument()->oligoobject->length+1;
		InvalidateRect(NULL);
		GetDocument()->Frame->UpdateWindow();
...
void COligoView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if (point.y>oligoline&&point.y<sequenceline) {
		...
		if (uni) {
			...
			ct.numofbases = GetDocument()->oligoobject->length;
		}
		else ct.numofbases = 2*GetDocument()->oligoobject->length+3;

		ct.allocate(ct.numofbases);
		strcpy(ct.ctlabel[1],"Oligowalk oligomer #");
		itoa(current,character,10);
		strcat(ct.ctlabel[1],character);
		strcat(ct.ctlabel[1],"\n");
		for (i=1;i<=GetDocument()->oligoobject->length;i++) {
			ct.numseq[ct.numofbases-i+1]=complement(current+i-1,&GetDocument()->oligoobject->ct);
			ct.nucs[ct.numofbases-i+1] = (tobase(ct.numseq[ct.numofbases-i+1]))[0]; 
			if (!uni) {
				ct.numseq[(ct.numofbases-3)/2-i+1]=complement(current+i-1,&GetDocument()->oligoobject->ct); 	
				ct.nucs[(ct.numofbases-3)/2-i+1] =(tobase(ct.numseq[(ct.numofbases-3)/2-i+1]))[0];
			}
		}
		if (!uni) {
		...
		sortstructures (&ct);
		ctout (&ct,"%%oligo_walk_oligomer.ct");
