...
UINT DynProc( LPVOID pParam ){    
	...
	for (i=0;i<pObject->maxtracebacks;i++)  align[i] = new short [pObject->ct1->numofbases+1];

	
	constraints = false;
	if (pObject->ct1->npair>0) constraints=true;
	if (pObject->ct2->npair>0) constraints=true;
	if (pObject->ct1->nforbid>0) constraints=true;
	if (pObject->ct2->nforbid>0) constraints=true;
	if (pObject->ct1->nnopair>0) constraints=true;
	if (pObject->ct2->nnopair>0) constraints=true;
	if (pObject->ct1->nmod>0) constraints=true;
	if (pObject->ct2->nmod>0) constraints=true;
	if (pObject->ct1->ngu>0) constraints=true;
	if (pObject->ct2->ngu>0) constraints=true;
	...
	if (pObject->m == -99) {
		...
		allowed_alignments = new bool *[pObject->ct1->numofbases+1];
		for (i=0;i<=pObject->ct1->numofbases;i++) {
			allowed_alignments[i] = new bool [pObject->ct2->numofbases+1];	
	
		}

		// Needed for having nucleotide sequences as c strings.
		pObject->ct1->nucs[pObject->ct1->numofbases + 1] = 0;
		pObject->ct2->nucs[pObject->ct2->numofbases + 1] = 0;
		...
		calculate_coinc_probs_env(pObject->ct1, pObject->ct2, NULL, allowed_alignments, pObject->datapath, pObject->forcealign);
	...
	ctout(pObject->ct1,pObject->ctoutfile);
	ctout(pObject->ct2,pObject->ctoutfile2);
...
void CDynalign::NewSequences() {
	...
	openseq(&GetDynDocument()->ct,m_sequencename.GetBuffer(0));
	openseq(&GetDynDocument()->ct2,m_sequence2name.GetBuffer(0));
	...
	if ((GetDynDocument()->ct.numofbases)>1200) {
   			structwindow=20;
			alignwindow=3;
	}
	else if ((GetDynDocument()->ct.numofbases)>800) {
   			structwindow=15;
			alignwindow=3; 
	}
	else if ((GetDynDocument()->ct.numofbases)>500) {
   			structwindow=11;
           alignwindow=3; 
	}
	else if ((GetDynDocument()->ct.numofbases)>300) {
   			structwindow=7;
			alignwindow=2; 
	}
	else if ((GetDynDocument()->ct.numofbases)>120) {
   			structwindow=5;
			alignwindow=1;
            
	}
	else if ((GetDynDocument()->ct.numofbases)>50) {
   			structwindow=3;
			alignwindow=1;
...
void CDynalign::OnConstraintsforalignmentForcealignment()
{
	...
	if (forcealign==NULL) {
		...
		forcealign[0]=new short [GetDynDocument()->ct.numofbases+1];
		forcealign[1]=new short [GetDynDocument()->ct2.numofbases+1];
		for (index=1;index<=GetDynDocument()->ct.numofbases;index++) forcealign[0][index]=0;
		for (index=1;index<=GetDynDocument()->ct2.numofbases;index++) forcealign[1][index]=0;
		


	}

	alignforcedialog = new CAlignForceDialog(forcealign,GetDynDocument()->ct.numofbases,GetDynDocument()->ct2.numofbases,this);
...
void CDynalign::OnConstraintsforalignmentShowcurrentalignmentconstraints()
{
	...
	if (forcealign==NULL) {
		...
		message="";
		for (index=1;index<=GetDynDocument()->ct.numofbases;index++) {
...
void CDynalign::OnConstraintsforalignmentSavealignment()
{
	...
	if (filedialog->DoModal()==IDOK) {
		...
		out << GetDynDocument()->ct.numofbases << "\n";
		out << GetDynDocument()->ct2.numofbases << "\n";
		for (index=1;index<=GetDynDocument()->ct.numofbases;index++) {

			out << forcealign[0][index]<<"\n";
		}
		for (index=1;index<=GetDynDocument()->ct2.numofbases;index++) {
...
void CDynalign::OnConstraintsforalignmentRestorealignment()
{
	...
	if (AfxMessageBox( "This will erase all current alignmnet constraints.\nContinue?", 
		...
		if (filedialog->DoModal()==IDOK) {
			...
			in >> temp;
			if (temp!=GetDynDocument()->ct.numofbases) {
			...
			in >> temp;
			if (temp!=GetDynDocument()->ct2.numofbases) {
