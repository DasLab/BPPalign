	...
	if (filedialog->DoModal()==IDOK) {
		...
		for (i=1;i<ct->numofbases;i++) {
			for (j=i+1;j<=ct->numofbases;j++) {
				...
				if (((PlotDoc *)GetDocument())->array[j][i]>0) {  //a value > 0 is stored for pairs that cannot occur
				for (count=1;count<=8;count++) {

					//temp = 	pow(10,-((PlotDoc *)GetDocument())->array[j][i]);

					if (count==1) {
						if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>=.99) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
						}
					}
					else if (count==2) {
							if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>=.97) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
							}
					}
					else if (count==3) {
						if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>=.95) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
						}
					}
					else if (count==4) {
						if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>=.90) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
						}
					}
					else if (count==5) {
						if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>=.80) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
						}
					}
					else if (count==6) {
						if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>=.70) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
						}
					}
					else if (count==7) {
						if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>=.60) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
						}
					}
					else if (count==8) {
						if (pow(10,-((PlotDoc *)GetDocument())->array[j][i])>.50) {
						ct->basepr[count][i]=j;
						ct->basepr[count][j]=i;
						}
					}
		...
		ctout(ct,(filedialog->GetPathName()).GetBuffer(0));
		...
		for (i=1;i<ct->numofbases;i++) {
			for (j=i+minloop+1;j<=ct->numofbases;j++) {
				if (plotdoc->array[j][i]<=plotdoc->colorrange[plotdoc->colors]&&plotdoc->array[j][i]>=plotdoc->colorrange[0])
					out << i << "\t"<<j<<"\t"<<(((PlotDoc *)GetDocument())->array[j][i])<<"\n"; 
