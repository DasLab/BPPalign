...
UINT BPProc( LPVOID pParam ){    
	...
	tempct->allocate(pObject->ct->numofbases);
	strcpy(tempct->ctlabel[1],"temp\n");
	tempct->numofbases = pObject->ct->numofbases;

	for (i=1;i<=pObject->ct->numofbases;i++) {
	...
	for (structures=1;structures<=pObject->ct->numofstructures;structures++) {

		//initialize all base pairs as unallowed:
		for (i=0;i<=pObject->ct->numofbases;i++) {
			for (j=i+1;j<=pObject->ct->numofbases;j++) {
		...
		for (i=1;i<=pObject->ct->numofbases;i++) {
			if (pObject->ct->basepr[structures][i]>i) {
				tempct->tem[pObject->ct->basepr[structures][i]][i] = true;
			}
		}
		...
		for (i=1;i<=pObject->ct->numofbases;i++) {
			pObject->ct->basepr[structures][i]=tempct->basepr[1][i];

		}

		//Also copy the energy back to ct
		pObject->ct->energy[structures]=tempct->energy[1];
...
void CRemovePseudo::OnBnClickedStart()
{
	...
	openct(&GetFoldDocument()->ct,m_ctinname.GetBuffer(0));
	(GetFoldDocument()->ct).allocatetem();

	//add a new line to the end of the title
	for (i=1;i<=(GetFoldDocument()->ct).numofstructures;i++)
