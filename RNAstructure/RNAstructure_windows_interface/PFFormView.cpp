...
UINT PFProc( LPVOID pParam ){    
	...
	if (pObject->subfold&&!pObject->ct->intermolecular) {
		...
		for (i=bases;i>minloop+1;i--) {
			...
			pfunction((pObject->ct),data,0,savename);
...
void CPFFormView::OnBnClickedSequencebutton()
{
	...
	if (filedialog->DoModal()==IDOK) {
		...
		openseq (&(GetFoldDocument()->ct),sequencename.GetBuffer(10));	
...
void CPFFormView::OnForceCurrent()
{
   ...
   if (GetFoldDocument()->ct.ndbl>0) {
   	strcat (message,"Bases forced double:\n");
      for (it=1;it<=GetFoldDocument()->ct.ndbl;it++) {
      	itoa(GetFoldDocument()->ct.dbl[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
   ...
   if (GetFoldDocument()->ct.npair>0) {
   	strcat(message,"Forced Base Pairs:\n");
   	for (it = 1; it<=GetFoldDocument()->ct.npair;it++) {

 			itoa(GetFoldDocument()->ct.pair[it][0],temp,10);
         strcat(message,"  ");
         strcat(message,temp);
         strcat(message," - ");
         itoa(GetFoldDocument()->ct.pair[it][1],temp,10);
         strcat(message,temp);
         if (it%5==0) strcat(message,"\n");
		 else strcat(message,", ");
      }
   ...
   if (GetFoldDocument()->ct.nnopair>0) {
   	strcat (message,"Bases forced single:\n");
      for (it=1;it<=GetFoldDocument()->ct.nnopair;it++) {
      	itoa(GetFoldDocument()->ct.nopair[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
         if (it%5==0) strcat(message,"\n");
		 else strcat(message,", ");
   ...
   if (GetFoldDocument()->ct.ngu>0) {
   	strcat (message,"U's in GU pairs:\n");
      for (it=0;it<GetFoldDocument()->ct.ngu;it++) {
      	itoa(GetFoldDocument()->ct.gu[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
   ...
   if (GetFoldDocument()->ct.nmod>0) {
   	strcat (message,"Bases accessible to chemical modification:\n");
      for (it=1;it<=GetFoldDocument()->ct.nmod;it++) {
      	itoa(GetFoldDocument()->ct.mod[it],temp,10);
         strcat(message,"   ");
         strcat(message,temp);
         if (it%5==0) strcat(message,"\n");
		 else strcat(message,", ");
   ...
   if (GetFoldDocument()->ct.nforbid>0) {
		strcat (message,"Prohibited Base Pairs=");
		for (it = 0; it<GetFoldDocument()->ct.nforbid;it++) {

 			itoa(GetFoldDocument()->ct.forbid[it][0],temp,10);
			strcat(message,"  ");
			strcat(message,temp);
			strcat(message," - ");
			itoa(GetFoldDocument()->ct.forbid[it][1],temp,10);
			strcat(message,temp);
			if (it%5==0) strcat(message,"\n");
			else strcat(message,", ");
		}
	...
	if (GetFoldDocument()->ct.ndbl==0&&GetFoldDocument()->ct.npair==0&&
		GetFoldDocument()->ct.nnopair==0&&GetFoldDocument()->ct.ngu==0&&
		GetFoldDocument()->ct.nmod==0&&GetFoldDocument()->ct.min_gu==0&&
